...
- 컴퓨터 하드웨어 프로세서가 이해하는 기계수준 언어로 컴파일하여 실행 (언어 : C 등)
...
- 객체(object)는 프로그램 실행 중에 생성되어 메모리에 거주하는 실체이다.
...
- 애플리케이션(Application)은 컴퓨터 운영체제에서 실행되는 모든 소프트웨어를 통칭한다. 우리말로는 응용프로그램이라고도 한다.
...
- `javax.swing`은 Java Standard API Library에서 제공하는 Graphic User Interface(GUI) 라이브러리 패키지(package) 이다. (Java API Documentation 사이트 방문하여 확인)
