...
## Arithmetic and Variables (2020/9/17)
...
힌트: 어떤 수를 `9`로 나눈 나머지가 `0`과 같으면, 그 수는 `9`로 나누어 떨어진다.
...
예를 들어 9로 나누어 떨어지는 `423`, `-4725`, '19287'는 
모두 자리별 숫자의 합이 각각 `9`, `18`, `27`로 9로 나누어 떨어진다.

`-99999`와 `+99999` 사이의 정수를 프로그램 인수로 받아,
자리별 숫자의 합이 `9`로 나누어 떨어지는지 알려주는 애플리케이션을 작성하자.
